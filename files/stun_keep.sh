...
KEEPALIVE() {
	...
	for SERVER in $(cat /tmp/SiteList.txt); do
		...
		[[ $StunMode =~ nft ]] || while :; do echo -ne "HEAD / HTTP/1.1\r\nHost: $SERVER\r\nConnection: keep-alive\r\n\r\n"; sleep $StunInterval; done | eval socat - tcp4:$SERVER:80,connect-timeout=2,reuseport,sourceport=$STUN_ORIG_PORT$STUN_IFACE >/dev/null 2>&1
		LOG $SERVER 已断开，保持 $(($(date +%s)-$START)) 秒
