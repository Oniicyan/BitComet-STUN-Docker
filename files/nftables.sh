...
UPDATE_HTTPS() {
	...
	for SERVER in $(awk -F / '{print $3}' HttpsTrackers.txt); do
		...
		for IP in $(getent ahosts $DOMAIN | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | sort | uniq); do
			...
			nft add element ip STUN BTTR_HTTPS { $IP . $PORT } || LOG $DOMAIN($IP):$PORT 解析失败
		done
	done
...
[ "$StunModeHttps" = 0 ] || {
	...
	nft insert rule ip STUN BTTR ip daddr 127.0.0.1 meta l4proto tcp $OFFSET_HTTP_GET 0x474554202f616e6e6f756e63653f goto BTTR_HTTP
}
